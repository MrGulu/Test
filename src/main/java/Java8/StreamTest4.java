...
//                .min((x, y) -> Double.compare(x.getSalary(), y.getSalary()));
         ...
         * 为Comparator<? super T> comparator；所以用下面两种方式都可以，一种是用Lambda
         * 表达式实现这个接口，另一个是简写的方法引用。
         ...
         *  2.上面是需要对一个个实体类中的工资进行比较，比下面的操作更深入一层，所以前两种方式可以
         * 使用Lambda表达式实现这个接口。也可以使用最后一种方式，这是Comparator接口的一个静态方法（JDK1.8之后接口中可以存在静态方法）
